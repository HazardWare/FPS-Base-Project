...
@export var enable_fall_kick : bool = true ## Toggles the view kick from landing.
@export var enable_damage_kick : bool = true
@export var enable_recoil : bool = true
...
@export_group("View Roll and Tilt")
...
@export var fall_time : float = 0.3 ## How long the fall kick effect will last.
...
func calculate_view_offset(delta : float) -> void:
	...
	if speed > 0.1 and player.is_on_floor():
		...
		_step_timer = 0.0
	var bob_sin = sin(_step_timer * 2.0 * PI) * 0.5 # 0.5 reduces the magnitude of the sin wave.
...
func add_screen_shake(amount : float, seconds : float) -> void:
	if _screen_shake_tween:
		_screen_shake_tween.kill() # Rather violent function name

	_screen_shake_tween = create_tween()
	_screen_shake_tween.tween_method(
			update_screen_shake.bind(amount), 0.0, 1.0, seconds).set_ease(Tween.EASE_OUT)
